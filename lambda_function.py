...
def send_email(to_addr: str | None, subject: str, body: str) -> None:
    ...
    try:
        ...
        print(f"[EMAIL] sent ok to={to_addr} subject={subject} messageId={resp.get('MessageId')}")
    except Exception as e:
        # Never break matching/clearing because email failed
        try:
            log("error", "Email send failed", to=to_addr, subject=subject, error=str(e))
        except Exception:
